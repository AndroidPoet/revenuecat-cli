...
var WebhooksCmd = &cobra.Command{
	...
	Long:  `List and create webhook integrations for your RevenueCat project.`,
}
...
func init() {
	...
	createCmd.Flags().StringVar(&webhookURL, "url", "", "webhook URL")
...
func runList(cmd *cobra.Command, args []string) error {
	...
	path := fmt.Sprintf("/projects/%s/integrations/webhooks", client.GetProjectID())
...
func runCreate(cmd *cobra.Command, args []string) error {
	...
	if cli.IsDryRun() {
		output.PrintInfo("Dry run: would create webhook with URL '%s'", webhookURL)
		return nil
	}
	...
	path := fmt.Sprintf("/projects/%s/integrations/webhooks", client.GetProjectID())
	if err := client.Post(ctx, path, body, &webhook); err != nil {
		return err
	}

	output.PrintSuccess("Webhook created successfully")
	return output.Print(webhook)
}
